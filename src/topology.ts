...
export interface SwitchFacts {
  ...
  ports: Array<{ port: number; link_up: boolean; type: string; speed_code: number | null }>;
...
function inferHierarchy(endpointA: Endpoint, endpointB: Endpoint) {
  ...
  if (a === 0 && b === 0) {
    return {
      determined: false,
      reason: "Neither link port has learned foreign MAC addresses (idle link or empty MAC tables); cannot infer direction.",
    };
  }
  if (a === b) {
    return {
...
export function analyzeTopology(facts: SwitchFacts[]): Record<string, unknown> {
  ...
  for (let i = 0; i < facts.length; i += 1) {
    for (let j = i + 1; j < facts.length; j += 1) {
      ...
      if (aSeesB.length > 0 && bSeesA.length > 0) {
        ...
        const aSfp = a.ports.filter((p) => p.link_up && p.type === "sfp");
        const bSfp = b.ports.filter((p) => p.link_up && p.type === "sfp");
        if (aSfp.length === 1 && bSfp.length === 1) {
          aPort = aSfp[0]!.port;
          bPort = bSfp[0]!.port;
          method = "inferred: the single active SFP/10G port on each switch (MAC tables did not confirm adjacency)";
          confidence = "low";
        }
      }

      if (aPort === null || bPort === null) {
        notes.push(`Could not determine a link between ${a.target} and ${b.target}. ${!a.mac_table.readable || !b.mac_table.readable ? "At least one MAC address table was not readable. " : ""}Verify both switches are logged in and physically connected.`);
        continue;
      }
  ...
  return {
    switches: facts.map((f) => ({
      ...
      mac_table_readable: f.mac_table.readable,
      mac_table_entries: f.mac_table.count,
